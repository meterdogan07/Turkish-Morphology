...
class Seq2SeqDataLoader(Dataloader):
    def __init__(
        ...
        if self.nb_attr > 0:
            ...
            self.attr_c2i = {c: i + len(self.source_c2i) for i, c in enumerate(self.source[-self.nb_attr :])
            }
        else:
        ...
        print()
        print(self.attr_c2i.keys())
        print(self.attr_c2i.values())
        print()
...
class SIGMORPHON2017Task1(Seq2SeqDataLoader):
    ...
    def _iter_helper(self, file):
        for lemma, word, tags in self.read_file(file):
            ...
            trg.append(self.target_c2i[EOS])
            print(lemma, " ",tags)
            print(len(src), "source: ", src)
            print(word)
            print(len(trg), "target: ", trg)
            print("")
            yield src, trg
