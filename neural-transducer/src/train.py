...
class Trainer(BaseTrainer):
    ...
    def set_args(self):
        ...
        parser.add_argument('--max_seq_len', default=32000, type=int)
        parser.add_argument('--max_decode_len', default=32000, type=int)
    ...
    def decode(self, mode, batch_size, write_fp, decode_fn):
        ...
        cnt = 0
        sampler, nb_batch = self.iterate_batch(mode, batch_size)
        with open(f"{write_fp}.{mode}.txt", "w") as fp:
            fp.write(f"{mode} \n")
            for src, src_mask, trg, trg_mask in tqdm(
                sampler(batch_size), total=nb_batch
            ):
                ...
                for p, t, loss in zip(pred, trg, losses):
                    ...
                    t = self.data.decode_target(t)
                    fp.write(f'\n{cnt}-prediction:  \n{" ".join(p)}\n')
                    fp.write(f'{cnt}-target:  \n{" ".join(t)}\n')
                    fp.write(f'loss: {loss.item()} ,dist: {dist}\n')
                    cnt += 1
