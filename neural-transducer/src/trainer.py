...
class BaseTrainer(object):
    ...
    def reload_and_test(self, model_fp, best_fp, batch_size, decode_fn):
        ...
        # TRAIN SET
        self.calc_loss(TRAIN, batch_size, -1)
        self.logger.info("decoding train set")
        results = self.decode(TRAIN, batch_size, f"{model_fp}.decode", decode_fn)
        if results:
            for result in results:
                self.logger.info(f"TRAIN {result.long_desc} is {result.res} at epoch -1")
            results = " ".join([f"{r.desc} {r.res}" for r in results])
            self.logger.info(f'TRAIN {model_fp.split("/")[-1]} {results}')
    ...
    def run(self, start_epoch, decode_fn=None):
        ...
        for epoch_idx in range(start_epoch, max_epochs):
            ...
            with torch.no_grad():
                ...
                trainloss = self.calc_loss(TRAIN, params.bs, epoch_idx)
                eval_res2 = self.evaluate(TRAIN, params.bs, epoch_idx, decode_fn)
