...
def collate_morse(batch):
    max_width = max([img[0].size(1) for img in batch])
    
    indexes = [len(img[0]) for img in batch]

    src_batch = np.concatenate([
        F.pad(img[0], [0, max_width - img[0].size(1), 0, 0])
        for img in batch])

    max_width = max([img[1].size(1) for img in batch])

    trg_batch = np.concatenate([
        F.pad(img[1], [0, max_width - img[1].size(1), 0, 0])
        for img in batch])
    return (
        torch.tensor(src_batch).transpose(0, 1),
        torch.tensor(trg_batch).transpose(0, 1),
        torch.tensor(indexes)
    )

def collate_morse2(batch):
    max_height = max([img[0].size(0) for img in batch])
    max_width = max([img[0].size(1) for img in batch])


    src_batch = np.stack([
        F.pad(img[0], [0, max_width - img[0].size(1), 0, max_height - img[0].size(0)])
        for img in batch])
        

    max_height = max([img[1].size(0) for img in batch])
    max_width = max([img[1].size(1) for img in batch])

    trg_batch = np.stack([
        F.pad(img[1], [0, max_width - img[1].size(1), 0, max_height - img[1].size(0)])
        for img in batch])
    
    return (
        torch.tensor(src_batch),
        torch.tensor(trg_batch)
    )
...
def create_masks(src, trg, device):
    ...
    if trg is not None:
        ...
        if trg.is_cuda:
            np_mask
        trg_mask = trg_mask.to(device) & np_mask.to(device)
    else:
        trg_mask = None
