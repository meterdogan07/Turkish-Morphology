...
model = torch.load("/kuacc/users/merdogan18/hpc_run/TrMor2023/versions/non_context/deneme.pt", map_location=torch.device('cpu'))
...
with torch.no_grad():
    ...
    print(f"\tTest Loss: {train_loss:.3f} | Levenstein: {train_edit:.3f} | Train Acc: {train_acc:.3f} | Train Word Acc: {train_word_acc:.3f}")
