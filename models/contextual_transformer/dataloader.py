...
class Vocab:
    def __init__(self, data, start_token="<s>", eos_token="</s>", pad_token="<p>", unk_token="<unk>", space_token=" ", max_size=64):
        ...
        encoder_default = {pad_token:0, start_token:1, eos_token:2, unk_token:3, space_token:4}
        decoder_default = {0:pad_token, 1:start_token, 2:eos_token, 3:unk_token, 4:space_token}
        # Surface dictionaries
        surf_encoder = dict(**encoder_default)
        surf_decoder = {0:pad_token, 1:start_token, 2:eos_token, 3:unk_token, 4:space_token}
        # Feature dictionaries
        feat_encoder = dict(**encoder_default) 
        feat_decoder = {0:pad_token, 1:start_token, 2:eos_token, 3:unk_token, 4:space_token}
        ...
        for j, tag in enumerate(np.sort(list(set(tags)))):
            feat_encoder[tag] = j+5
            feat_decoder[j+5] = tag
            
        for j, surf in enumerate(np.sort(list(set(lemmas)))):
            surf_encoder[surf] = j+5
            surf_decoder[j+5] = surf
