...
from model.morse_layers import WordEncoder, ContextEncoder, WordDecoder, ContextDecoder
...
class Morse(nn.Module):
    ...
    def accuracy2(self, output_logits, targets, epoch):
        # output_logits: (B, T, vocab_size), targets: (B,T)
        surf_vocab = self.surf
        feat_vocab = self.feat
        ...
        # Log predictions into file
        correct_predictions = []; wrong_predictions = []; prediction_res = []; target_res = []
        target_str = ""
        pred_str = ""
        if epoch % 1 == 0:
            for i in range(B):
                
                target = "".join([("+"+feat_vocab[e.item()]) if len(feat_vocab[e.item()])>1 else feat_vocab[e.item()] for e in targets[i]])
                pred = "".join([("+"+feat_vocab[e.item()]) if len(feat_vocab[e.item()])>1 else feat_vocab[e.item()] for e in pred_tokens[i]])

                if '</s>' not in target: # ignore dummy inputs with full of pads
                    continue
                
                target = target[:target.index('</s>')] #take until eos
                pred = pred[:len(target)]
                if target != pred:   
                    wrong_predictions.append('target: %s pred: %s' % (target, pred))
                else:
                ...
                if(i-1 % 64 == 0 and i>1):
                    prediction_res.append(pred_str)
                    target_res.append(target_str)
                    pred_str = ""
                    target_str = ""
                    
        return  num_correct, num_total, num_wrong, wrong_predictions, correct_predictions, prediction_res, target_res