...
task_no = 0
tasks = ["Seq2Seq", "Seq2SeqAttention", "Transformer"]
logger_file   = 'exp__deneme'            # Add ExpNUMBER !!!         
...
train_loader  = DataLoader(dataset_train, batch_size=args.batchsize, shuffle=True, collate_fn=collate_wordbased)
...
for epoch in range(args.epochs):
    ...
    logger.info(f"Epoch: {epoch+1:02} | Time {epoch_mins}m {epoch_secs}s | lr: {scheduler.get_last_lr()[0]}")
...
logger.info(f"\tTrain Loss: {train_loss:.3f} | Levenstein: {train_edit:.3f} | Train Acc: {train_acc:.3f} | Train Word Acc: {train_word_acc:.3f}")
