...
parser.add_argument("--model_name_or_path", default="ai-forever/mGPT")#'ai-forever/mGPT')
    ...
    with open(data_path, 'r', encoding="utf-8") as reader:
        ...
        for sentence in data_all:
            for word in sentence:
                ...
                data['text_a'] = lemma.lower()
                data['tgt_text'] = root.lower() + "+" + tags
                data["guid"] = ix
                data["text_b"] = ""
                data["meta"] = {}
                data["label"] = None
...
tot_step  = len(train_dataloader)*6
...
run = "xlast2"
ff = open(run+"logger_run.txt", 'w' ,encoding = "utf-8")
...
for epoch in range(6):
    ...
    with open(run+"out_validation"+str(epoch)+"_run"+".txt",'w',encoding = "utf-8") as f:
...
with open(run+"out_test"+str(epoch)+"_run"+".txt",'w',encoding = "utf-8") as f:
    for i,sent in enumerate(generated_sentence):
        f.write(target[i] + " => "+sent+"\n")
    f.write('acc: %.3f' % (correct/len(generated_sentence)))
    print('acc: %.3f' % (correct/len(generated_sentence)))
